  ...
  <article class="leaderboard">
    <header>
      ...
      <h1 class="leaderboard__title">
        <span class="leaderboard__title--top">Reaction Game</span>
        <span class="leaderboard__title--bottom">Leaderboard</span>
      </h1>
    </header>

    <main class="leaderboard__profiles">
      <article class="leaderboard__profile --player" v-for="(player, index) in players" :key="player.id">
        <h1 class="leaderboard__rank">#{{ 1 + index }}</h1>
        <span class="leaderboard__name">{{ player.username }}</span>
        <span class="leaderboard__value">{{ player.score.toFixed(2) }}<span>ms</span></span>
      </article>
      <article class="leaderboard__profile" v-if="players.length == 0">
        <h1 class="leaderboard__rank"></h1>
        <span class="leaderboard__name">No players</span>
...
export default {
  ...
  methods: {
    fetchLeaderboard() {
      api.get('/leaderboard').then((res) => (this.players = res.data));
    }
  },
  beforeMount() {
    console.debug('Fetching leaderboard');
...
.leaderboard__profile {
  ...
  cursor: pointer;
  transition: transform 0.2s cubic-bezier(0.7, 0.98, 0.86, 0.98), box-shadow 0.25s cubic-bezier(0.7, 0.98, 0.86, 0.98);
  background-color: #fff;
}

.leaderboard__profile.--player:hover {
  box-shadow: 0 5px 14px -1px rgba(51, 51, 51, 0.4);
}
